...
class MutableObjectProvider : public MutableObjectProviderInterface {
 ...
 private:
  ...
  // Protects the `written_so_far_` and `write_acquired_` maps.
  ...
  // out-of-order chunks. The sender fires all chunk RPCs concurrently, so a later chunk
  // may arrive before the first chunk's WriteAcquire completes. Other threads must wait
  // until this is true before calling GetObjectBackingStore().
