...
class MutableObjectProvider : public MutableObjectProviderInterface {
 ...
 private:
  static constexpr int kMaxPushRetries = 5;
  ...
  /// Pushes a mutable object to a single reader with retry-on-failure.
  /// On failure, retries the entire object push with backoff up to kMaxPushRetries times.
  void PushToReaderWithRetry(
  ...
  // Protects written_so_far_, write_acquired_, highest_completed_version_,
  // and received_offsets_.
  absl::Mutex written_so_far_lock_;
  // Tracks how many bytes have been received so far for the current write of each object.
  std::unordered_map<ObjectID, uint64_t> written_so_far_
      ABSL_GUARDED_BY(written_so_far_lock_);
  // Tracks whether WriteAcquire has been called AND completed for each object.
  // Ensures WriteAcquire is called exactly once per version. Other threads wait until
  // this is true before calling GetObjectBackingStore().
  ...
  // version == highest_completed + 1 is the active version that can write.
  ...
  std::unordered_map<ObjectID, int64_t> highest_completed_version_
      ABSL_GUARDED_BY(written_so_far_lock_);
  // Tracks which chunk offsets have been received for the current write of each object.
  // Used to avoid double-counting written_so_far_ when a whole-object retry resends
  // chunks that were already received. Cleared on version completion.
  std::unordered_map<ObjectID, std::unordered_set<uint64_t>> received_offsets_
      ABSL_GUARDED_BY(written_so_far_lock_);
