...
class DeploymentState:
    ...
    def _stop_or_update_outdated_version_replicas(self, max_to_stop=math.inf) -> bool:
        ...
        if gang_config is not None:
            ...
            # Every replica in a gang deployment should have gang_context set
            # (via start() or recovered via check_ready()), so we expect all
            # restart candidates to be groupable.
            ...
            logger.info(
                f"Stopping {code_version_changes} gang replicas of {self._id} with outdated versions."
            )
    ...
    def _check_and_stop_outdated_version_replicas(self) -> bool:
        ...
        # For gang deployments, ensure rollout_size is at least a multiple of gang_size
        # so that we always stop and start complete gangs, respecting gang boundaries.
        gang_config = self.get_gang_config()
        if gang_config is not None:
